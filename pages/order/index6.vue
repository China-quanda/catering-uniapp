...
<script>
	export default {
		data() {
			return {
				optIndex:0,  //左边菜单值，同时也为上一个页面传过来的值（传过来精确在第几行并使样式为active）
				itemId: '', // 右边scroll-view用于滚动的id（自动滚动定位左边）
      ...
      },
      {
		...
		onLoad(opt){
			...
			if(opt.index){
				this.itemId = "item"+opt.index;
				this.optIndex = opt.index;
			}
...
<style scoped>
	...
	.u-tab-item-active::before {
		...
		width: 36rpx;
		height: 36rpx;
		background: url('../../static/activeIcon.png') no-repeat;
	...
	.serchInput {
		...
		height: 60rpx;
		z-index: 1;
		padding: 12rpx 0 12rpx 96rpx;
		display: inline-block;
		border-radius: 70rpx;
		background: #F5F6FA;
		box-shadow: 0rpx 10rpx 32rpx 2rpx rgba(0,0,0,0.04);
	}
